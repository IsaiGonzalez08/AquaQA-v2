...
const generateAlertEvents = (): AlertEvent[] => [
  { id: "1", sensor: "temperature", startTime: "08:30", endTime: "09:15", startIndex: 20, endIndex: 23, type: "warning", message: "Temperatura elevada" },
  { id: "2", sensor: "ph", startTime: "14:00", endTime: "14:45", startIndex: 42, endIndex: 45, type: "critical", message: "pH fuera de rango crítico" },
  { id: "3", sensor: "turbidity", startTime: "18:30", endTime: "19:00", startIndex: 60, endIndex: 62, type: "warning", message: "Turbidez alta" },
];
  ...
  data.slice(0, 50).forEach((point, index) => {
    SENSORS.forEach(sensor => {
      ...
      const matchingEvent = events.find(e => 
        e.sensor === sensor.id && index >= e.startIndex && index <= e.endIndex
      );
      if (matchingEvent) {
      ...
      {payload.map((entry, index) => {
        ...
        return (
          <div key={index} className="flex items-center gap-2 text-sm">
            ...
            <span className="font-medium">{entry.value.toFixed(2)}{sensor?.unit}</span>
          </div>
        );
      })}
    </div>
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Análisis detallado del comportamiento histórico de los sensores
          </p>
        </div>
                ...
                <div className="flex items-center gap-2">
                  <div className="relative flex-1">
                    ...
                    <input
                      ...
                      className="bg-background w-full rounded-md border py-2 pl-10 pr-3 text-sm"
                    />
                  </div>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <select
                    ...
                    className="bg-background w-full appearance-none rounded-md border py-2 pl-3 pr-10 text-sm"
                  >
          ...
          <CardContent>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <ComposedChart data={displayData}>
                  ...
                  {selectedSensors.includes("temperature") && (
                    <>
                      <ReferenceLine y={18} stroke="#3b82f6" strokeDasharray="5 5" label={{ value: "Temp Min", fill: "#3b82f6", fontSize: 10 }} />
                      <ReferenceLine y={28} stroke="#3b82f6" strokeDasharray="5 5" label={{ value: "Temp Max", fill: "#3b82f6", fontSize: 10 }} />
                    </>
                  )}

                  {/* Alert event areas */}
                  {showEvents && alertEvents
                    .filter(e => selectedSensors.includes(e.sensor))
                    .map(event => (
                      <ReferenceArea
                        key={event.id}
                        x1={displayData[Math.max(0, event.startIndex - (zoomDomain?.start ?? 0))]?.time}
                        x2={displayData[Math.min(displayData.length - 1, event.endIndex - (zoomDomain?.start ?? 0))]?.time}
            ...
            {showEvents && (
              <div className="mt-4 flex flex-wrap gap-4 border-t pt-4">
                {alertEvents.filter(e => selectedSensors.includes(e.sensor)).map(event => {
              ...
              <Card key={sensorId}>
                ...
                <CardContent>
                  ...
                  <div className="mt-2 flex justify-between text-xs text-muted-foreground">
                    <span>Rango óptimo: {sensor.minOptimal} - {sensor.maxOptimal} {sensor.unit}</span>
                  </div>
                </CardContent>
              </Card>
            );
            ...
            <Card className="lg:col-span-2">
              <CardHeader className="pb-2">
                <CardTitle className="flex items-center gap-2 text-lg">
                  ...
                  Correlación: {SENSORS.find(s => s.id === selectedSensors[0])?.name} vs {SENSORS.find(s => s.id === selectedSensors[1])?.name}
                </CardTitle>
              </CardHeader>
              <CardContent>
                <div className="h-72">
            ...
            return (
              <Card key={sensorId}>
                ...
                <CardContent>
                  <div className="h-64">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={data}>
                        ...
                        />
                        <YAxis tick={{ fontSize: 10 }} tickLine={false} label={{ value: "Frecuencia", angle: -90, position: "insideLeft", fontSize: 11 }} />
                        <Tooltip
                  ...
                  <div className="mt-2 flex justify-between text-xs text-muted-foreground">
                    <span>Líneas verdes: límites óptimos ({sensor.minOptimal} - {sensor.maxOptimal} {sensor.unit})</span>
                  </div>
                </CardContent>
              </Card>
            );
